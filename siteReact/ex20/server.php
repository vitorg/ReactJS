<?php
//unset( $_SESSION['db_react_ex11'] );
session_start(); // to use $_SESSION['variable']
$db = null; // GLOBAL
if( isset($_SESSION['db_react_ex11'])  ) {
	$db = $_SESSION['db_react_ex11'];
}
else {
	$db = array();
  ...
  $_SESSION['db_react_ex11'] = $db; // save to session (to be avaliable on the next request)
}
/* CONVERT JSON Array of objects to PHP Array of objects
var a = [];
var result = "";
...
function response_to_POST() {
	if( $_POST ) { // if data was sent
		...
		$_SESSION['db_react_ex11'] = $GLOBALS['db']; // save to session (to be avaliable on the next request)
	}
  	echo '{"data": ' . json_encode($GLOBALS['db']) . '}';
    //echo '{"data": ' . json_encode( array( array("category" => "Sporting Goods", "price" => "$49.99", "stocked" => "true", "name" => "Football"), array("category" => "Sporting Goods", "price" => "$49.99", "stocked" => "true", "name" => "Football2") ) ) . '}';
}
